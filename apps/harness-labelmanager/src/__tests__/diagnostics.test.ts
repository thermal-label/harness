 ...
 * diagnostics block therefore carries only `connectStatus` /
 * `prePrintStatus` / `postPrintStatus` / `finalStatus`, and the
 * decoded status carries no `details[]` / `battery` (graceful
 * degradation — the shell renders no diagnostics panel and no battery
 * glyph for it).
 */
