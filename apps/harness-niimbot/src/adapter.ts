...
export const adapter: DriverAdapter<NiimbotDevice, NiimbotMedia> = {
  ...
  buildReport: ({ device, identity, primarySession, mocked }) => {
    ...
    // usedPaper / labelType / capacity / rawHex`. An earlier draft of
    // this surfacing used the pre-rename names (`usedLengthMm` /
    // `totalLengthMm`); keep this shape in sync with the driver.
